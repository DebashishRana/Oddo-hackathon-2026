...
const proxyAuthRequest = async (
  ...
  if (path.startsWith("google/")) {
    return NextResponse.redirect(new URL(`/api/auth/${path}${request.nextUrl.search}`, API_BASE_URL), 307);
  }

  try {
    const targetUrl = new URL(`/api/auth/${path}${request.nextUrl.search}`, API_BASE_URL);
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Authentication service is unavailable. Check that the backend server is running and the API URL is correct.",
      },
      { status: 502 }
    );
  }
